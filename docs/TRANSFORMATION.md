...
| `policy_violations` | 0 |
| `python_modules` | 86 |
| `note` | أرقام ما قبل T1؛ قارنها بـ docs/baseline.json |
...
- قرار مُتخذ أثناء التنفيذ: الاسم `audit` كان محجوزًا لمهيّئ المسار القديم، فأُعيد تسميته `legacy-audit` بلا تغيير سلوك، وانتقل الاسم إلى الأمر الذي يدقّق فعلًا
- RUN.md: مصنوعة تملكها مرحلة validate، تقول ما فُحص وما لم يُفحص وسببه قبل أي نتيجة
