...
class Artifact:
    ...
    required: bool = True
    absent_when: str = ''
    record_absent_because: str = ''   # a document with no machine twin must say why, not stay silent
...
ARTIFACTS = (
    ...
    Artifact('DATA-MODEL.md', 'bundles', DOCUMENT, 'Persisted shapes and their migrations', 16, 200,
             record_absent_because='rendered straight from the fact sets in facts/; the full detail already lives there, so a second copy would be a second source of truth'),
    ...
    Artifact('KPI.md', 'bundles', DOCUMENT, 'What to measure to know the transformation worked', 36, 120,
             record='sustainability.json'),
    Artifact('EXECUTION-GUIDE.md', 'execution_guide', DOCUMENT,
             'Literal task-by-task instructions for an independent implementer', 37, 300,
             record='plan.json'),
    Artifact('BASELINE.md', 'validate', DOCUMENT, 'The debt accepted when the baseline was pinned, and what has closed since', 38, 120, record='baseline/baseline.json', required=False,
    ...
    Artifact('product-review.json', 'compose', RECORD, 'The summary a caller reads back',
             checked=False),
    # Named as the twin of a document, so they belong in the ledger too: a record a document
    # points at and nobody declares is a file no stage is responsible for.
    Artifact('facts/external.json', 'engines', RECORD, 'Every external engine finding, normalised',
             required=False, absent_when='no engine was requested, or none is installed'),
    Artifact('facts/policy.json', 'policy', RECORD, 'Every architecture-policy violation in full',
             required=False, absent_when='the project declares no architecture policy'),
    Artifact('baseline/baseline.json', 'validate', RECORD, 'The pinned debt baseline this run is judged against',
             required=False, absent_when='no baseline is pinned in this report directory; it is '
                                         'written by `eaos baseline pin`, not by the run itself'),
    Artifact('transform-plan.json', 'transform', RECORD, 'Every transform stage in full'),
    Artifact('predictions.json', 'transform', RECORD,
             'What each stage claims it will change, written down before anything changes',
             required=False,
             absent_when='the plan proposes no stage, so there is nothing to predict about'),
    ...
    Artifact('index.html', 'site', DOCUMENT, 'The same records, browsable', 50, 100000, required=False,
             record_absent_because='it renders the other documents; every one of them names its own twin',
