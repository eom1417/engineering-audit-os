...
def document(sets, language='ar'):
    from .correlate import clusters, CONTESTED
    ...
    doc.table([words['engine'], words['evaluated'], words['findings']],
              [[name, ', '.join(f'{kind}:{state}' for kind, state in sorted(evaluated.get(name, {}).items())) or '—',
    ...
    contested = [(cluster, kind, detail) for cluster in built
                 for kind, detail in sorted(cluster['corroboration'].items()) if detail['verdict'] == CONTESTED]
    if contested:
        doc.section(words['contested'])
        doc.table([words['place'], words['kind'], words['asserted'], words['denied']],
                  [[cluster['place'], kind, ', '.join(detail['asserted_by']), ', '.join(detail['denied_by'])]
