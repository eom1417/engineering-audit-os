# TASK-024 — pkg/command/command.go: calls an external service with no timeout or retry — load blocker (unprotected_dependency)

> claim: CLM-560 · pattern: load_blocker · priority: 0.0729
...
| Command | Expected |
|---|---|
| human review / مراجعة هندسية | CLM-560: CONFIRMED or REFUTED concerns the observation only; record requirement evidence and decide repair, retain, or blocked_missing_requirement. A timeout, retry policy or circuit breaker on the call, or evidence the call is local. |
