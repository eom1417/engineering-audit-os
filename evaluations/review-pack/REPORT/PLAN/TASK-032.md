# TASK-032 — 2 symbols share the same structure up to identifier names (internal/intent/constraints.go:888 .function_declaration, int

> claim: CLM-280 · pattern: canonicalize · priority: 0.0491
...
2 symbols share the same structure up to identifier names (internal/intent/constraints.go:888 .function_declaration, int

Editing the rule in one copy and not the other makes the paths disagree, and nothing in the code links them.

## Evidence

- facts: FACT-a576911ee0a3ca16
- probes: PRB-032
- falsifier: Evidence that the occurrences encode different rules that evolve for different reasons, which would make a single definition wrong rather than missing.

## Blast radius

- files: internal/intent/constraints.go, internal/mining/naming.go
- مستوردون مباشرون (42): internal/config/config.go, internal/diff/constraintattribution_test.go, internal/diff/constraintcredit_test.go, internal/docslint/inventory.go, internal/engine/conceptcensusreach_test.go, internal/engine/conceptedgematrix_test.go, internal/engine/conceptownsmethods_test.go, internal/engine/conceptrolesides_test.go … (+34)
...
| Command | Expected |
|---|---|
| human review / مراجعة هندسية | CLM-280: CONFIRMED or REFUTED concerns the observation only; record requirement evidence and decide repair, retain, or blocked_missing_requirement. Evidence that the occurrences encode different rules that evolve for different reasons, which would make a single definition wrong rather than missing. |
