# TASK-051 — 2 symbols share the same structure up to identifier names (internal/linkers/binders/messagingcontract/messagingcontract.

> claim: CLM-297 · pattern: canonicalize · priority: 0.0259
...
2 symbols share the same structure up to identifier names (internal/linkers/binders/messagingcontract/messagingcontract.

Editing the rule in one copy and not the other makes the paths disagree, and nothing in the code links them.

## Evidence

- facts: FACT-aea3736bf205476e
- probes: PRB-051
- falsifier: Evidence that the occurrences encode different rules that evolve for different reasons, which would make a single definition wrong rather than missing.

## Blast radius

- files: internal/linkers/binders/messagingcontract/messagingcontract.go, pkg/history/topology.go
...
| Command | Expected |
|---|---|
| human review / مراجعة هندسية | CLM-297: CONFIRMED or REFUTED concerns the observation only; record requirement evidence and decide repair, retain, or blocked_missing_requirement. Evidence that the occurrences encode different rules that evolve for different reasons, which would make a single definition wrong rather than missing. |
