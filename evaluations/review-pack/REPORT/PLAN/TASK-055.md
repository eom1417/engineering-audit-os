# TASK-055 — examples/cross-repo/api/server.go: calls an external service with no timeout or retry — load blocker (unprotected_depend

> claim: CLM-538 · pattern: load_blocker · priority: 0.0235
...
| Command | Expected |
|---|---|
| human review / مراجعة هندسية | CLM-538: CONFIRMED or REFUTED concerns the observation only; record requirement evidence and decide repair, retain, or blocked_missing_requirement. A timeout, retry policy or circuit breaker on the call, or evidence the call is local. |
