# TASK-065 — 5 symbols share the same structure up to identifier names (pkg/bootstrap/bootstrap.go:94 .method_declaration, pkg/bootst

> claim: CLM-499 · pattern: canonicalize · priority: 0.0219
...
5 symbols share the same structure up to identifier names (pkg/bootstrap/bootstrap.go:94 .method_declaration, pkg/bootst

Editing the rule in one copy and not the other makes the paths disagree, and nothing in the code links them.

## Evidence

- facts: FACT-9956e40ede8af7b8
- probes: PRB-065
- falsifier: Evidence that the occurrences encode different rules that evolve for different reasons, which would make a single definition wrong rather than missing.

## Blast radius

- files: pkg/bootstrap/bootstrap.go
...
| Command | Expected |
|---|---|
| human review / مراجعة هندسية | CLM-499: CONFIRMED or REFUTED concerns the observation only; record requirement evidence and decide repair, retain, or blocked_missing_requirement. Evidence that the occurrences encode different rules that evolve for different reasons, which would make a single definition wrong rather than missing. |
