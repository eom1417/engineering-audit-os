# TASK-093 — 3 symbols share the same structure up to identifier names (internal/extractors/hclextractor/hcl.go:46 .function_declarat

> claim: CLM-398 · pattern: canonicalize · priority: 0.0172
...
3 symbols share the same structure up to identifier names (internal/extractors/hclextractor/hcl.go:46 .function_declarat

Editing the rule in one copy and not the other makes the paths disagree, and nothing in the code links them.

## Evidence

- facts: FACT-0d03dab3fced7031
- probes: PRB-093
- falsifier: Evidence that the occurrences encode different rules that evolve for different reasons, which would make a single definition wrong rather than missing.

## Blast radius

- files: internal/extractors/hclextractor/hcl.go, internal/extractors/tsextractor/ember.go, internal/extractors/tsextractor/ts.go
...
| Command | Expected |
|---|---|
| human review / مراجعة هندسية | CLM-398: CONFIRMED or REFUTED concerns the observation only; record requirement evidence and decide repair, retain, or blocked_missing_requirement. Evidence that the occurrences encode different rules that evolve for different reasons, which would make a single definition wrong rather than missing. |
